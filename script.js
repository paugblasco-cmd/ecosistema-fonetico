...
async function descargarFichaPDF() {
    // Detectar si estamos en materiales adaptados o en generador IA
    const cont =
        document.getElementById("mat-preview") ||
        document.getElementById("gen-result");

    if (!cont || !cont.innerHTML.trim() || cont.innerHTML.includes("Pensando")) {
    ...
    lineas.forEach(linea => {
        const trozos = doc.splitTextToSize(linea, 475);
...
async function enviarAudioSTT() {
  ...
  try {
    const res = await fetch("https://ecosistema-fonetico-backend.onrender.com/api/stt", {
      method: "POST",
      body: formData
    });
